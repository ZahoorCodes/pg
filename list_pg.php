...
if(!empty($_FILES["image"]["name"] && $_FILES["video"]["name"])  )
    ...
    //  $uploadedvideoname= $_FILES["video"]["name"];
      ...
      //$target_path_video="videos/".$videonewname;
			...
			{
                $sql= "INSERT INTO tbl_pg
                (pg_title,address,budget_per_month,
                contact,created_at,district_id,img,pg_description,landmark,locality_id,region_id,user_id,status,meal_type,video) VALUES
                ('$title','$address','$budget','$contact',now(),'$district','$target_path','$description','$landmark','$locality','$region','$user_id','2','$meal','$target_video')";
